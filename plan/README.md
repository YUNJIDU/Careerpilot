...
- 后台常驻 Worker、跨设备主动推送和无人值守调度。
- 登录第三方账号、绕过反爬或付费墙。
- 自动投递、自动提交、验证码绕过和不经确认的表单填写。
- 用 Multi-Agent 阻塞首发闭环。

## 当前状态与推荐阅读顺序

Stage 0–7 已按各自详细设计在本地完成，Stage 6 评测决定继续使用受控单 Agent。当前交付范围、测试和使用说明见
[P0–P3 交付文档](../docs/DELIVERY_P0_P3.md)。下一阶段是 Stage 8；进入前必须先提交 PostgreSQL、Worker、对象存储、备份恢复、导出删除、账户/租户隔离和合规的详细设计，不得直接把本地单用户边界暴露到公网。

...
- 登录第三方内容账号、绕过反爬或付费墙。
- 自动填表、自动投递和自动提交。
- Multi-Agent、长期记忆、多租户和商业化。Stage 6 仅实现受控单 Agent。
...
| 6 | 受控单 Agent（Multi-Agent 待评测） | 稳定工具模块 | 工具编排、人工审批和评测 | 后续 |
...
- 单用户可管理多份简历和多个邮箱账户。
- 数据统一位于 `data/` 目录体系。
- Backup、Restore 和 Delete 只定义接口，完整功能后置。
...
模型只能提取和整理信息，不得把推断写成用户事实或替用户作求职决策。
